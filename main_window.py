...
class FilesDisplay(Frame):
    ...
    def delete_row(self, file, button):
        MsgBox = messagebox.askquestion('Delete', 'Are you sure you want to permanently delete this file?',
                                           icon='warning')
        if MsgBox == 'yes':
            messagebox.showinfo(title="Success", message="The file has been deleted!")
    ...
    def archive_row(self, file, button):
        ...
        self.ref_dat = self.inventory.copy()

        for index, row in arch_build.iterrows():
            if row['Name'] != "":
                dash_index = row['Name'].find('-')-1
                ID = int(row['Name'][:dash_index])
                row_to_update = self.ref_dat[self.ref_dat['ID']==ID]
                ...
                print(self.ref_dat[self.ref_dat['ID']==ID])
                print(self.inventory[self.inventory['ID']==ID])
